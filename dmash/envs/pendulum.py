...
class PendulumEnv(gym.Env):
    """
    Copied from https://github.com/Farama-Foundation/Gymnasium/blob/main/gymnasium/envs/classic_control/pendulum.py
    Modified such that contexts are parameters, input actions are between -1 and 1, and reward is multi-objetive.
    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        reward_weights: List[float] = [1.0, 0.1, 0.001],
    ):
        self.max_speed = 8
        self.max_torque = 1.0
        self.dt = 0.05
        ...
        self._context_high = {
            ...
            "action_factor": np.inf,
            "action_disturbance": np.inf,
            "reward_weights": [np.inf, np.inf, np.inf],
        }
    ...
    def step(self, u):
        ...
        u = np.clip(
            np.array(u) * self.action_factor + self.action_disturbance,
            -self.max_torque,
            self.max_torque
        )[0] * self.torque_factor
        self.last_u = u  # for rendering
        reward = np.array([-angle_normalize(th)**2, -thdot**2, -u**2], dtype=np.float32)
        ...
        return self._get_obs(), scalar_reward, False, False, {}
