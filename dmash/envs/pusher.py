...
class PusherEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model_path.startswith(".") or model_path.startswith("/"):
            self.fullpath_local = model_path
        elif model_path.startswith("~"):
            self.fullpath_local = path.expanduser(model_path)
        else:
            self.fullpath_local = path.join(path.dirname(__file__), "assets", model_path)
        if not path.exists(self.fullpath_local):
            raise OSError(f"File {self.fullpath_local} does not exist")

        MujocoEnv.__init__(
            self,
            "point.xml",  # any valid file, it's not used
            frame_skip,
        ...
        self.model.body_mass[self.model.body("object").id] = object_mass
    ...
    def _initialize_simulation(
        ...
        model = mujoco.MjModel.from_xml_path(self.fullpath_local)
