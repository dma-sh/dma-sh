...
def update_physics(xml_string, gravity=9.81, actuator_strength=1.0, mass_torso=67.86, mass_toe_fr=1.07, paresis=0.0, swap_lr=-1.0, swap_rf=-1.0):
    """
    ...
    for geom in geoms:
        if geom.get("name") == "torso":
            geom.set("mass", str(mass_torso))
        if geom.get("name") == "toe_front_right":
            geom.set("mass", str(mass_toe_fr))

    # Update actuator for paresis
    actuators = mjcf.findall("./actuator/")
    for actuator in actuators:
        if (
            paresis == -2.0 and "front_left" in actuator.get("name")
        ) or (
            paresis == -1.0 and "front_right" in actuator.get("name")
        ) or (
            paresis == 1.0 and "back_right" in actuator.get("name")
        ) or (
            paresis == 2.0 and "back_left" in actuator.get("name")
        ):
            gear = actuator.get("gear")
            actuator.set("gear", str(0.0))
...
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, context_kwargs=None):
    ...
    swap_lr = context_kwargs.get("swap_lr", -1.0)
    swap_rf = context_kwargs.get("swap_rf", -1.0)
    task = Move(desired_speed=_WALK_SPEED, random=random, swap_lr=swap_lr, swap_rf=swap_rf)
...
@SUITE.add()
def fetch(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, context_kwargs=None):
...
class Move(base.Task):
    ...
    def __init__(self, desired_speed, random=None, swap_lr=-1.0, swap_rf=-1.0):
        ...
        self._swap_lr = swap_lr > 0.0
        self._swap_rf = swap_rf > 0.0
    ...
    def before_step(self, action, physics):
        ...
        if self._swap_lr:
            ...
            action = action[[1, 0, 3, 2]]
            action = action.flatten()
        if self._swap_rf:
            action = action.reshape(4, 3)
            action = action[[3, 2, 1, 0]]
            action = action.flatten()
