...
def update_physics(xml_string, spinner_damping=0.03, spinner_mass=2.45):
    """
    ...
    for body in bodies:
        if body.get("name") == "spinner":
            joints = body.findall(".//joint")
            for joint in joints:
                if joint.get("name") == "hinge":
                    joint.set("damping", str(spinner_damping))

    # Update mass for the spinner
    bodies = mjcf.findall("./worldbody/body")
    for body in bodies:
        if body.get("name") == "spinner":
            geoms = body.findall(".//geom")
            for geom in geoms:
                if geom.get("name") == "cap1":
                    geom.set("mass", str(spinner_mass / 2))
                elif geom.get("name") == "cap2":
                    geom.set("mass", str(spinner_mass / 2))
                if geom.get("name") == "spinner_decoration":
                    geom.set("mass", str(0.0))
...
def spin(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, context_kwargs=None):
