...
class LunarLander(gym.Env, EzPickle):
    r"""
    Copied from https://github.com/Farama-Foundation/Gymnasium/blob/433d7af1f9ca7aabb0247803da71f1cd6dd91266/gymnasium/envs/box2d/lunar_lander.py
    Modified such that env is by default continuous, contexts are parameters and never terminates.
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        if self.game_over or abs(state[0]) >= 1.0:
            terminated = True
            reward = -100 if not self.prev_terminated else 0
